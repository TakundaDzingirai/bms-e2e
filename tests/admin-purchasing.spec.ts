...
// Auth is now ENFORCED: purchasing and ALL subpaths are admin-only (ensureAdmin).
test.describe("Purchasing", () => {
  test("list: 200 typed { data, total } (admin)", async () => {
    const client = makeClient(await getAccessToken());
    const { data, response } = await client.GET("/api/admin/purchasing", {
  ...
  test("company: 200 typed company details (admin)", async () => {
    const client = makeClient(await getAccessToken());
    const { data, response } = await client.GET("/api/admin/purchasing/company");
    expect(response.status).toBe(200);
    expect(typeof data?.po_company_name).toBe("string");
  });

  test("products: 200 typed products[] for a vendor (admin)", async () => {
    const client = makeClient(await getAccessToken());
    const { data, response } = await client.GET("/api/admin/purchasing/products", {
  ...
  test("create PO: 400 with an admin token but no vendor_id", async () => {
    const client = makeClient(await getAccessToken());
